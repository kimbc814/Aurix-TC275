...
void motor_contor (float speed){
    if(speed < 0){
        ...
        //IfxPort_setPinState(DIR_B_PIN, IfxPort_State_high);
        g_PWM_DUTY_CYCLE = -speed;
    }
    else{
        IfxPort_setPinState(DIR_A_PIN, IfxPort_State_high);
        //IfxPort_setPinState(DIR_B_PIN, IfxPort_State_low);
        g_PWM_DUTY_CYCLE = speed;
    }
    setGptPWM_Duty();
}
void PID_encoder(void)
{
    ...
    pwm_output = (pwm_output <= 10 && pwm_output > 0)  ?  10 : pwm_output;
    pwm_output = (pwm_output <= -98) ? -98 : pwm_output;
    pwm_output = (pwm_output >= -10 && pwm_output < 0)  ?  -10 : pwm_output;
...
void init_GPT12_module(void)
{
    /* Enable the GPT12 module */
        ...
        //IfxPort_setPinModeOutput(DIR_B_PIN, IfxPort_OutputMode_pushPull, IfxPort_OutputIdx_general);
        IfxPort_setPinState(PWM_A_PIN, IfxPort_State_low);
        IfxPort_setPinState(DIR_A_PIN , IfxPort_State_high);//low전진 high 후진
        //IfxPort_setPinState(DIR_B_PIN , IfxPort_State_high);//low전진 high 후진
}
