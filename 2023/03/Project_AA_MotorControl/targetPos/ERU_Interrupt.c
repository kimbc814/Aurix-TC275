...
#define ENCODER_A_PIN              &MODULE_P02,1            //DIGITAL
#define ENCODER_B_PIN              &MODULE_P02,0            /* Pin that is toggled by pressing the board's button   */
...
void SCUERU_Int0_Handler(void)
{
    count++;
    encoderDistance++;
    if(IfxPort_getPinState(ENCODER_A_PIN) == 0){
        if(IfxPort_getPinState(ENCODER_B_PIN) == 1){
             encoderPos++;
        }
        else encoderPos--;
    }
/*
            ...
            if(IfxPort_getPinState(ENCODER_B_PIN) == 0){
                encoderPos++;
            }
            else encoderPos--;
        }
*/
    if(encoderDistance ==1){
        time_pre=timeNow()/1000;
    }
    else if(encoderDistance ==231){
        time = timeNow()/1000;
        rpm = (2*3.1415/(time-time_pre))*60;
        //kmPh = rpm *6.452*0.001885 ;
        encoderDistance=0;
    }
    //pos_cm=(((float)encoderPos/461)*6.452*3.14);
...
void initPeripheralsAndERU(void)
{
    ...
    IfxPort_setPinMode(ENCODER_A_PIN, IfxPort_Mode_inputPullUp);
    IfxPort_setPinMode(ENCODER_B_PIN, IfxPort_Mode_inputPullUp);            /* Initialize TRIGGER_PIN port pin  */
