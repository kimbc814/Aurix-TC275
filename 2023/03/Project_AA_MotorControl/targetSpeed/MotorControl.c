...
uint16 g_PWM_DUTY_CYCLE =0;   //pid 제어에서 자동으로 바뀜.             /* Duty cycle of the PWM signal in percentage           */
extern uint32 finalCntVal_main;
extern long encoderPos;
int timecnt=0;

//위치 제어
float32 target_pos=0; //스피드 제어로 자동으로 바뀜
float32 error=0;
float32 error_d=0;
float32 error_s=0;
float32 error_old=0;
float pwm_output = 0;
//속도 제어 타겟
float target_speed = 180;//max 255(7.85). max 180(5.54). uart로 받아옴.8
/*********************************************************************************************************************/
...
int  timeNow(void){
    ...
    if(g_cntOverflow == 0)
    {
        ...
        finalCntVal_main= finalCntVal = currentCntVal - g_previousCntVal; /* Total counter value calculation */
    }
    else
    {
...
void PID_encoder(void)
{
    target_pos += (target_speed/(50*7.85)) * vel_2_pulse;
    error = target_pos - encoderPos;
    error_s += error;
    ...
    pwm_output = (pwm_output >= 98)  ?  98 : pwm_output;
    pwm_output = (pwm_output <= 3 && pwm_output > 0)  ?  3 : pwm_output;
    pwm_output = (pwm_output <= -98) ? -98 : pwm_output;
    pwm_output = (pwm_output >= -3 && pwm_output < 0)  ?  -3 : pwm_output;
...
void init_GPT12_module(void)
{
    /* Enable the GPT12 module */
        ...
        IfxPort_setPinState(DIR_A_PIN , IfxPort_State_high);//low전진 high 후진
        //IfxPort_setPinState(DIR_A_PIN1 , IfxPort_State_low);//low전진 high 후진
}
