...
#include "IfxScuWdt.h"   // 'System Control Unit' and 'Watchdog Timer' header file.
...
int core0_main(void)
{
    ...
    initMultican();     //initialize CAN Communication.
    initLed();          //initialize LED.
    //transmitCanMessage();

    while(1)
    {
        transmitCanMessage();   /*Send CAN message .*/
        waitTime(IfxStm_getTicksFromMilliseconds(BSP_DEFAULT_TIMER, WAIT_TIME_50ms));    /* Wait 50 milliseconds            */
    }
    return (1);
}
