...
/*-----------------------------------------------------Includes------------------------------------------------------*/
/*********************************************************************************************************************/
#include "MULTICAN1.h" // 'MULTICAN1.h' header file.
...
IFX_INTERRUPT(canIsrRxHandler, 0, ISR_PRIORITY_CAN_RX); /* Execute the canIsrRxHandler when an interrupt occurs. Priority is 1  */
...
void canIsrTxHandler(void)
{
   ...
   blinkLED1(); /* blink LED1 when TX interrupt occur. */
}
...
{
    IfxMultican_Status readStatus; // declares a variable named readStatus of type IfxMultican_Status.

    /* Read the received CAN message and store the status of the operation */
    readStatus = IfxMultican_Can_MsgObj_readMessage(&g_multican.canDstMsgObj, &g_multican.rxMsg); //read CAN receive data
    ...
    } // If no data is received, wait for the next interrupt.
    ...
    } // If the data received is corrupted, wait for the next interrupt.
        ...
        ( g_multican.rxMsg.id == g_multican.txMsg.id ) )
    {
        ...
        IfxPort_setPinLow(g_led.led2.port, g_led.led2.pinIndex); // when receive correct RX data set low pin led2
    }

    blinkLED2(); /* blink LED2 when RX interrupt occur. */
}
...
void initMultican(void)
{
     ...
     * ==========================================================================================
     */
    IfxMultican_Can_initModuleConfig(&g_multican.canConfig, &MODULE_CAN); // Configuration Initialize CAN module

    g_multican.canConfig.nodePointer[TX_INTERRUPT_SRC_ID].priority = ISR_PRIORITY_CAN_TX; // Set TX priority is 2
    g_multican.canConfig.nodePointer[RX_INTERRUPT_SRC_ID].priority = ISR_PRIORITY_CAN_RX; // Set RX priority is 1

    IfxMultican_Can_initModule(&g_multican.can, &g_multican.canConfig);  //initialize CAN module
     ...
     * ==========================================================================================
     */
    IfxMultican_Can_Node_initConfig(&g_multican.canNodeConfig, &g_multican.can);                               //CAN node configuration

    //g_multican.canNodeConfig.loopBackMode = TRUE;
    g_multican.canNodeConfig.baudrate = 500000UL;                                                              // CAN baudrate 500kbps
    g_multican.canNodeConfig.nodeId = (IfxMultican_NodeId)((int)IfxMultican_NodeId_0);                         // CAN TX Node ID 0
    g_multican.canNodeConfig.rxPin     = &IfxMultican_RXD0B_P20_7_IN;                                          // Set RX pin
    g_multican.canNodeConfig.rxPinMode = IfxPort_InputMode_pullUp;
    g_multican.canNodeConfig.txPin     = &IfxMultican_TXD0_P20_8_OUT;                                          // Set TX pin

    IfxMultican_Can_Node_init(&g_multican.canSrcNode, &g_multican.canNodeConfig);                              // TX configuration initialize
    ...
    g_multican.canNodeConfig.baudrate = 500000UL;                                                              // CAN baudrate 500kbps
    g_multican.canNodeConfig.nodeId    = (IfxMultican_NodeId)((int)IfxMultican_NodeId_0);                      // CAN RX Node ID 0
    g_multican.canNodeConfig.rxPin     = &IfxMultican_RXD0B_P20_7_IN;                                          // Set RX pin
    g_multican.canNodeConfig.rxPinMode = IfxPort_InputMode_pullUp;                                             // Set pinmode input pullup
    g_multican.canNodeConfig.txPin     = &IfxMultican_TXD0_P20_8_OUT;                                          // Set TX pin


    IfxMultican_Can_Node_init(&g_multican.canDstNode, &g_multican.canNodeConfig);                              // RX configuration initialize
     ...
     * ==========================================================================================
     */
    IfxMultican_Can_MsgObj_initConfig(&g_multican.canMsgObjConfig, &g_multican.canSrcNode); //Initialize CAN message object

    g_multican.canMsgObjConfig.msgObjId = SRC_MESSAGE_OBJECT_ID; // Set message object id
    g_multican.canMsgObjConfig.messageId = CAN_MESSAGE_ID_TX; // Set RX id
    g_multican.canMsgObjConfig.frame = IfxMultican_Frame_transmit; // Set frame send
    g_multican.canMsgObjConfig.txInterrupt.enabled = TRUE; // CAN TX Interrupt enable
    g_multican.canMsgObjConfig.txInterrupt.srcId = TX_INTERRUPT_SRC_ID; // Set RX interrupt id
    ...
    g_multican.canMsgObjConfig.msgObjId = DST_MESSAGE_OBJECT_ID; // Set message object id
    g_multican.canMsgObjConfig.messageId = CAN_MESSAGE_ID_RX; // Set RX id
    g_multican.canMsgObjConfig.frame = IfxMultican_Frame_receive; // Set frame receive
    g_multican.canMsgObjConfig.rxInterrupt.enabled = TRUE; // CAN RX Interrupt enable
    g_multican.canMsgObjConfig.rxInterrupt.srcId = RX_INTERRUPT_SRC_ID; // Set RX interrupt id
...
void transmitCanMessage(void)
{
    ...
    IfxMultican_Message_init(&g_multican.rxMsg,
                             ...
                             INVALID_DATA_VALUE,
                             INVALID_DATA_VALUE,
                             g_multican.canMsgObjConfig.control.messageLen); // Initialize the RX message object with invalid ID and data values

    /* Initialization of the TX message */
    IfxMultican_Message_init(&g_multican.txMsg,
                             g_multican.canMsgObjConfig.messageId, // 0x91
                             dataLow, // 0xC0CAC01A
                             dataHigh, // 0xBA5EBA11
                             g_multican.canMsgObjConfig.control.messageLen); // Initialize TX message object with the provided ID, data, length specified in the configuration
    ...
    {
    }  // Wait until the CAN message is sent successfully
}
...
void initLed(void)
{
    ...
    IfxPort_setPinHigh(g_led.led1.port, g_led.led1.pinIndex); // Set pin high LED1 at start
    IfxPort_setPinHigh(g_led.led2.port, g_led.led2.pinIndex); // Set pin high LED2 at start
...
void blinkLED1(void)
{
   ...
   IfxPort_setPinLow(g_led.led1.port, g_led.led1.pinIndex); // Set pin Low LED1
}
void blinkLED2(void)
{
    ...
    IfxPort_setPinLow(g_led.led2.port, g_led.led2.pinIndex); // Set pin Low LED2
}
