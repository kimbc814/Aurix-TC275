...
#define MCP79411_EEPROM_ADDRESS 0xae                            /* 174  7 bit slave device address for reading from EEPROM
                                                                   of MCP79411 is 0b1010111 which is 0x57.          */
#define ADDRESS_OF_MAC_ADDRESS  0xae                            /* Location of EUI-48 node address (MAC address)    */
...
uint8 g_macAddr[LENGTH_OF_MAC_ADDRESS] = {0,};    /* Global parameter for 6-byte EUI-48 MAC address   */
uint8 Mod =0;
uint16 Value=0;
//uint8 Success_receive_data=0;
struct CONTROL DataControl;
union
{
  uint16 check_sum;
  uint8 b[2];
...
void check_MODBUSCRC16(){
  if((g_macAddr[0]==0xae) && (g_macAddr[1]==0xa4)){
          if(g_macAddr[2]== 'I'){
              crc_16_val.check_sum = CRC16_MODBUS(g_macAddr, 6);
               if((crc_16_val.b[1] == g_macAddr[7]) && (crc_16_val.b[0] == g_macAddr[6])){
                   Mod=DataControl.mod = g_macAddr[3];
                   DataControl.a.value8[0] = g_macAddr[4];
                   DataControl.a.value8[1] = g_macAddr[5];
                   DataControl.a.value8[2] = 0;
                   DataControl.a.value8[3] = 0;
                  Value=DataControl.a.value16[0];
                  DataControl.Seccess=1;
              }
               else DataControl.Seccess=0;
          }
          else DataControl.Seccess=0;
      }
  else DataControl.Seccess=0;
