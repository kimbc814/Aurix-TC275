...
void motor_mod(void){
    ...
    MES.IDE=0;
    MES.DLC=8;
    old_mod=DataControl.mod;
    switch(DataControl.mod){
        case 1://front velocity
        case 2://front velocity
        case 5://front velocity
            motor_velocity();
            break;
        case 3://front velocity
        case 4://front velocity
            motor_position();
            break;
...
void motor_control(void){
    switch(DataControl.mod){
        case 1://front velocity
            ...
            MES.Data[4]=DataControl.a.value8[0];
            MES.Data[5]=DataControl.a.value8[1];
            MES.Data[6]=0x00;
            MES.Data[7]=0x00;
            CAN_send(&MES);
            tmp_flag=1;
            break;
        case 2://reverse velocity
            motor_velocity();
            for(int i=0; i<8; i++){
                MES.Data[i]=Speed[i];
            }
        ...
        case 3:
            if(tmp_flag==0){
                MES.ID=0x601;
                   ...
                   MES.Data[4] = 0x01;
                   CAN_send(&MES);
            tmp=DataControl.a.value32*10;
            DataControl.a.value32=tmp;
            for(int i=0; i<8; i++){
                  MES.Data[i]=position[i];
            }
            MES.Data[4]=DataControl.a.value8[0];
            MES.Data[5]=DataControl.a.value8[1];
            MES.Data[6]=DataControl.a.value8[2];
            MES.Data[7]=DataControl.a.value8[3];
            CAN_send(&MES);
            for(int i=0;i<8;i++){
                 MES.Data[i] = Enable[i];
             }
