...
uint8 g_rxData[SIZE] = {''};
uint8 protocal_test[SIZE1] = { 0, };
uint8 parsing_rx_buffer[8] = { 0, };
uint8 send_IO=0;
uint8 ch_input[8]={0,};
/* Size of the message */
Ifx_SizeT g_count = sizeof(g_txData);
Ifx_SizeT p_count = sizeof(protocal_test);
Ifx_SizeT r_count = sizeof(parsing_rx_buffer);
...
void tx_channel(void)
{

    protocal_test[0] = 174;                 //PC
    protocal_test[1] = 184;                 //IPD_controller
    protocal_test[2] = 'i';                 // PID
    protocal_test[3] = send_IO;            // data number
    protocal_test[4] = send_IO;            // data number
...
void rx_buffer(void)
{
    if((parsing_rx_buffer[0]==0xb8) && (parsing_rx_buffer[1]==0xae))
    {
        if(parsing_rx_buffer[2]== 'O')
        {
            crc_16_val.check_sum = CRC16_MODBUS(parsing_rx_buffer, 5);

            if((crc_16_val.b[1] == parsing_rx_buffer[6]) && (crc_16_val.b[0] == parsing_rx_buffer[5]))
            {
                data_num = parsing_rx_buffer[3];
                O_data = parsing_rx_buffer[4];
            }
        }
    }
}
...
int receive_ASCLIN_UART_message(void)
{
    ...
    count++;
    IfxAsclin_Asc_read(&g_ascHandle, parsing_rx_buffer , &r_count, TIME_INFINITE);
    return parsing_rx_buffer[0];
}
...
void change_rdata_type(uint8 indata){
    int tempdata=indata;
    if(tempdata>=128){tempdata-=128;ch_input[7]=1;}else{ch_input[7]=0;}
    if(tempdata>=64){tempdata-=64;ch_input[6]=1;}else{ch_input[6]=0;}
    if(tempdata>=32){tempdata-=32;ch_input[5]=1;}else{ch_input[5]=0;}
    if(tempdata>=16){tempdata-=16;ch_input[4]=1;}else{ch_input[4]=0;}
    if(tempdata>=8){tempdata-=8;ch_input[3]=1;}else{ch_input[3]=0;}
    if(tempdata>=4){tempdata-=4;ch_input[2]=1;}else{ch_input[2]=0;}
    if(tempdata>=2){tempdata-=3;ch_input[1]=1;}else{ch_input[1]=0;}
    if(tempdata>=1){tempdata-=1;ch_input[0]=1;}else{ch_input[0]=0;}
}
void change_sdata_type(){
    send_IO=0;
    for(int i=0;i<8;i++){
        if(ch_input[i]==1){
            send_IO+=pow(2,i);
