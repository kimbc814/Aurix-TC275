...
int adcbadc1,adcbadc2;
void tx_channel(int iiinput_num1,int aaadc_value)
{
    ...
    protocal_test[4] = iiinput_num1;            // data number

    crc_16_val.check_sum = CRC16_MODBUS(protocal_test, 5);
    adc_16_val.aaaadc_value=aaadc_value;

    protocal_test[5] = crc_16_val.b[1];
    protocal_test[6] = crc_16_val.b[0];
    protocal_test[7] = adcbadc1=adc_16_val.badc[1];
    protocal_test[8] = adcbadc2=adc_16_val.badc[0];
    protocal_test[9] = '\n';
...
void send_ASCLIN_UART_message(int iinput_num1,int aadc_value)
{
    send_count++;
    tx_channel(iinput_num1,aadc_value);
    IfxAsclin_Asc_write(&g_ascHandle, protocal_test, &p_count, TIME_INFINITE);  /* Transmit data via TX */
}

unsigned char receive_ASCLIN_UART_message(void)
{
    rx_buffer();
