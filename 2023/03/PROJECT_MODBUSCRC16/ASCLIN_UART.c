...
void buffer_rx(void)
{
    if((parsing_buffer_rx[0]==0xb8) && (parsing_buffer_rx[1]==0xae))
    {
        if(parsing_buffer_rx[2]== 'O')
        {
            crc_16_val.check_sum = CRC16_MODBUS(parsing_buffer_rx, 5);

            if((crc_16_val.b[1] == parsing_buffer_rx[6]) && (crc_16_val.b[0] == parsing_buffer_rx[5]))
            {
                data_num = parsing_buffer_rx[3];
                O_data = parsing_buffer_rx[4];
            }
        }
    }
}
...
{
    count++;
    IfxAsclin_Asc_read(&g_ascHandle, parsing_buffer_rx, &r_count, TIME_INFINITE);  /* Transmit data via TX */
}
