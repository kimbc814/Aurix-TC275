#include <Wire.h> //아두이노에서 I2C통신을 사용하기 위해서 사용된다.
#include <MsTimer2.h>//loop문 안의 내용과 관계없이 타이머 인터럽트가 발생하면 아무런 딜레이 없이 호출돤다.
#include <avr/wdt.h>//주기적으로 프로그램 실행 상태를 감시하며 정해진 시간 내에 프로그램 실행이 완료되지 않으면 시스템 리셋을 수행한다.
#define SLAVE 4// 자신의 슬레이브 주소를 설정해 줍니다.(슬레이브 주소에 맞게 수정해야 한다.)
...
void setup()//Slave 
{
  ...
  Wire.onReceive(Ring_Buff); //마스터가 SLAVE 장치에게 데이터를 보내면, 그 데이터를 수신하여 처리하는 함수
  Wire.onRequest(sendToMaster);// 마스터의 데이터 전송 요구가 있을 때 처리할 함수 등록
  Serial.begin(115200);
  MsTimer2::set(2,Timer1);//set(타이머 시간, 인터럽트 루틴으로 정해진 타이머마다 실행될 함수 이름)
  MsTimer2::start();//타이머 인터럽트 실행
  Serial.println("start");
  wdt_disable(); // Watchdog 타이머 비활성화
  wdt_enable(WDTO_30MS); // Watchdog 타이머 15ms로 설정하여 활성화
...
void Ring_Buff()// Ring_Buffer의 함수
{
...
void Timer1() 
{
    ...
    if(digitalRead(PIN_WATCHDOG_IN) == HIGH && kkk>10){wdt_reset();kkk=0;}else{kkk++;} //watchdog 입력 핀 상태를 확인, HIGH인 경우 watchdog 타이머 리셋.
    ADC_.ADC_VALUE=analogRead(A0); //A0에서 아날로그 입력 값을 읽어 변수 ADC_.ADC_VALUE에 저장.
    //Serial.println(ADC_.ADC_VALUE);
    for(int i=0;i<8;i++) //스위치 입력 핀(SW_IN)의 상태를 읽어 배열 SW_DATA에 저장.
    {
    ...
    B_pin_data[1]=digitalRead(BUTTON1);//버튼 상태 체크 후 배열에 저장.
